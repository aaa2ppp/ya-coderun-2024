...
// находит все ребра графа которые участвуют в циклах
func findLoopEdges(graph [][]int) [][2]int {
	...
	dfs = func(node, prev int) bool {
		...
		if n, ok := loop_starts[node]; ok {
			loop_count -= n
			delete(loop_starts, node)
		}

		return in_loop && loop_count > input_loop_count
	}
