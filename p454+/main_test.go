...
func Test_run(t *testing.T) {
	...
	tests := []struct {
		...
		// Только ввод/вывод проверить (остальные неоднозначные). Cм. Test_solve
		{
			"4",
			args{strings.NewReader(`3 3 6
1 1
...
func generateTest(n, m int, seed int64) testArgs {
	...
	N1 := 100 * n * m
	N2 := 100 * int(math.Sqrt(float64(n*m)))
	ab := make([][2]int, N2*3/200)
	for t := 1; t < len(toppings); t++ {
		for p := 1; p < len(people); p++ {
			if t == people[p].dissatisfaction || rand.Intn(N1) < N2 {
				if people[p].wants[t] {
	...
	rand.Shuffle(len(ab), func(i, j int) {
		ab[i], ab[j] = ab[j], ab[i]
	})
...
func Test_solve(t *testing.T) {
	...
	tests := []struct {
		...
		{
			"-1",
			args{2, 3, [][2]int{{1, 1}, {1, 2}, {1, 3}, {2, -1}, {2, -2}, {2, -3}}},
			false,
			true,
		},
		...
		},
		{
			"7",
			args{
				5, 5, [][2]int{{2, 2}, {4, 4}, {5, -5}, {5, 1}, {1, -5}, {3, 4}, {4, 3}, {5, 4}, {2, -4}, {2, -1}, {3, -3}, {2, -3}, {1, 1}},
			},
			true,
			true,
		},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if tt.wantOk && !checkResult(tt.args.n, tt.args.m, tt.args.ab, got) {
				t.Errorf("solve() got = %v: bad result", got)
			}
			if gotOk != tt.wantOk {
...
func Fuzz_solve(f *testing.F) {
	...
	f.Fuzz(func(t *testing.T, seed int) {
		...
		if 0 <= seed && seed < len(tests) {
			...
			args := generateTest(n, n, int64(seed))
			// sort.Slice(args.ab, func(i, j int) bool {
			// 	return args.ab[i][1] < args.ab[j][1]
			// })
			tt = test{
				strconv.Itoa(seed),
				args,
...
func Benchmark_solve(b *testing.B) {
	// Большего размера generateTest не сгенирирует. Матрица 100000x100000 это очень много...
	args := generateTest(10000, 10000, time.Now().UnixNano())
	b.Run("xxx", func(b *testing.B) {
		for i := 0; i < b.N; i++ {
			solve(args.n, args.m, args.ab)
		}
	})
}
